...
class NextDns:
    ...
    async def get_settings(self, profile_id: str) -> Settings:
        ...
        return Settings(
            ...
            block_9gag=services.get(API_9GAG, False),
            block_amazon=services.get(API_AMAZON, False),
            block_blizzard=services.get(API_BLIZZARD, False),
            block_dailymotion=services.get(API_DAILYMOTION, False),
            block_discord=services.get(API_DISCORD, False),
            block_disneyplus=services.get(API_DISNEYPLUS, False),
            block_ebay=services.get(API_EBAY, False),
            block_facebook=services.get(API_FACEBOOK, False),
            block_fortnite=services.get(API_FORTNITE, False),
            block_hulu=services.get(API_HULU, False),
            block_imgur=services.get(API_IMGUR, False),
            block_instagram=services.get(API_INSTAGRAM, False),
            block_leagueoflegends=services.get(API_LEAGUEOFLEGENDS, False),
            block_messenger=services.get(API_MESSENGER, False),
            block_minecraft=services.get(API_MINECRAFT, False),
            block_netflix=services.get(API_NETFLIX, False),
            block_pinterest=services.get(API_PINTEREST, False),
            block_primevideo=services.get(API_PRIMEVIDEO, False),
            block_reddit=services.get(API_REDDIT, False),
            block_roblox=services.get(API_ROBLOX, False),
            block_signal=services.get(API_SIGNAL, False),
            block_skype=services.get(API_SKYPE, False),
            block_snapchat=services.get(API_SNAPCHAT, False),
            block_spotify=services.get(API_SPOTIFY, False),
            block_steam=services.get(API_STEAM, False),
            block_telegram=services.get(API_TELEGRAM, False),
            block_tiktok=services.get(API_TIKTOK, False),
            block_tinder=services.get(API_TINDER, False),
            block_tumblr=services.get(API_TUMBLR, False),
            block_twitch=services.get(API_TWITCH, False),
            block_twitter=services.get(API_TWITTER, False),
            block_vimeo=services.get(API_VIMEO, False),
            block_vk=services.get(API_VK, False),
            block_whatsapp=services.get(API_WHATSAPP, False),
            block_xboxlive=services.get(API_XBOXLIVE, False),
            block_youtube=services.get(API_YOUTUBE, False),
            block_zoom=services.get(API_ZOOM, False),
        )
